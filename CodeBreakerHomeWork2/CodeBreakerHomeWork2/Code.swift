...
struct Code {
    ...
    init(kind: Kind, pegsCount: Int, gameKind: CodeBreaker.Kind) {
        self.gameKind = gameKind
        self.kind = kind
        self.pegsCount = pegsCount
        self.pegs = Array(repeating: Peg.pegMissing, count: pegsCount)
    }
    ...
    mutating func reset() {
        pegs = Array(repeating: Peg.pegMissing, count: pegsCount)
    }
